...
async def leaderboard(
    ...
    """
    Таблица лидеров.
    - by=xp   → сортировка по опыту (по умолчанию)
    - by=pvp  → сортировка по ELO-рейтингу
    Возвращает топ-N + позицию текущего пользователя.
    """
    ...
    # Find current user's rank if they're outside the top-N
    ...
    if current_user:
        ...
        if not me_in_top:
            count_result = await db.execute(
                select(User)
                .where(User.is_active.is_(True), order_col > getattr(current_user, "elo" if by == "pvp" else "xp"))
            )
